...
<div class="dashboard-container">
    ...
    <nav class="sidebar">
        ...
        <a href="{{ route('dashboard.books') }}" class="nav-item-link {{ request()->routeIs('dashboard.books') ? 'active' : '' }}"><i class="fa-solid fa-book"></i> Kelola Buku</a>
        <a href="{{ route('dashboard.articles') }}" class="nav-item-link {{ request()->routeIs('dashboard.articles') ? 'active' : '' }}"><i class="fa-regular fa-newspaper"></i> Kelola Artikel</a>
    ...
    <main class="main-content position-relative">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 mt-2">
            <h3 class="fw-bold mb-0">{{ $page_title ?? 'Kelola Data' }}</h3>
            <a href="{{ route('dashboard.posts.create') }}" class="btn btn-primary rounded-pill px-4 fw-bold"><i class="fa-solid fa-plus me-2"></i> Tambah Baru</a>
        </div>
                ...
                <table class="table align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                <span class="fw-bold d-block text-main">{{ $post->title }}</span>
                                <div class="small text-secondary text-truncate" style="max-width: 280px;">{{ $post->excerpt }}</div>
                            </td>
                            <td><span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-25">{{ $post->category }}</span></td>
                            <td class="text-secondary">{{ $post->author }}</td>
                            <td class="text-end">
                                <a href="{{ route('dashboard.posts.edit', $post->id) }}" class="btn btn-sm btn-warning text-white rounded-circle"><i class="fa-solid fa-pen"></i></a>
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5 text-secondary">
                                <div class="bg-secondary bg-opacity-10 rounded-circle d-inline-flex p-4 mb-3"><i class="fa-regular fa-folder-open fa-2x"></i></div>
                                <p class="mb-0">Data tidak ditemukan untuk kategori ini.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
